...
async function renderList() {
  ...
  const slots = ["Orario da definire", "07:30", "08:00", "08:30", "09:00", "09:30", "10:00", "10:30", "11:00", "11:30"];
  const maxPerSlot = 12;
  ...
  slots.forEach((slot, i) => {
    ...
    if (bookings.length > 0) {
      ...
      if (slot === "Orario da definire") {
        titleText = "Orario da definire";
        // Per "Orario da definire" mostriamo solo quante ne abbiamo inserite, senza barra e senza limite
        infoHtml = ``;
      } else {
        let nextSlot = slots[i + 1] || slot;
        titleText = `${slot} - ${nextSlot}`;
        // Per tutte le altre fasce mostriamo tutto: conteggio rimanenti e progress bar
        infoHtml = `
      ...
      <form id="editForm">
        ...
        <label style="margin-top: 1rem;">Fascia Oraria
          <select id="editTimeSlot">
            <option value="Orario da definire">Orario da definire</option>
